...
func TestDoublyLinkedList_Add_Append(t *testing.T) {
	list := NewDoublyLinkedList[int]()
	...
	if list.tail.Data != 3 {
		t.Errorf("tail = %d, expected 3", list.tail.Data)
	}
	// Backward chain must include the appended node.
	if list.tail.Prev.Data != 2 {
		t.Errorf("tail.Prev = %d, expected 2", list.tail.Prev.Data)
	}
}

func TestDoublyLinkedList_Add_EmptyList(t *testing.T) {
	list := NewDoublyLinkedList[int]()
	...
	if list.head != list.tail {
		t.Error("head and tail should be the same node after seeding")
	}
